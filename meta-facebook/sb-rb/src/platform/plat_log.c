...
void error_log_event(uint16_t error_code, bool log_status)
{
	...
	if (error_code == HAMSA_MFIO22_ERROR_CODE) {
		...
		if ((current_time_get - hamsa_remote_err_last_time_stamp) > 10000) {
			...
			LOG_INF("same asic remote temp error within 10s, current_time_get: %lld, hamsa_remote_err_last_time_stamp: %lld", current_time_get, hamsa_remote_err_last_time_stamp);
			return;
		}
	}
	else if (error_code == MEDHA0_MFIO24_ERROR_CODE) {
		...
		if ((current_time_get - medha0_remote_err_last_time_stamp) > 10000) {
			...
			LOG_INF("same asic remote temp error within 10s, current_time_get: %lld, medha0_remote_err_last_time_stamp: %lld", current_time_get, medha0_remote_err_last_time_stamp);
			return;
		}
	}
	else if (error_code == MEDHA1_MFIO24_ERROR_CODE) {
		...
		if ((current_time_get - medha1_remote_err_last_time_stamp) > 10000) {
			...
			LOG_INF("same asic remote temp error within 10s, current_time_get: %lld, medha1_remote_err_last_time_stamp: %lld", current_time_get, medha1_remote_err_last_time_stamp);
			return;
		}
	}
