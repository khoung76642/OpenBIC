		...
		if (err_code_caches[i] == error_code) {
			if (log_status == LOG_ASSERT) {
				...
				if (error_code == HAMSA_MFIO22_ERROR_CODE) {
					...
					if (error_code != CLK_100MHZ_ERR_CODE && error_code != CLK_312_5MHZ_ERR_CODE
						&& error_code != CLK_BUF0_100M_LOSB_PLD_ERR_CODE && error_code != CLK_BUF1_100M_LOSB_PLD_ERR_CODE
				...
				LOG_INF("Duplicate error_code(log deassert): 0x%x, log_status: %d", error_code,
					log_status);
				return;
			}
		}
