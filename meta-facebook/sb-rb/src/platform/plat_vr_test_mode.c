...
const vr_test_mode_setting_t vr_test_mode_table[] = {
	...
	{ VR_RAIL_E_ASIC_P0V85_MEDHA0_VDD, 14500, 14500, 200, 940, 940, 595, 940 },
	{ VR_RAIL_E_ASIC_P0V85_MEDHA1_VDD, 14500, 14500, 200, 940, 940, 595, 940 },
	{ VR_RAIL_E_ASIC_P0V9_OWL_E_TRVDD, 170, 170, 200, 1130, 1130, 630, 1130 },
	...
	{ VR_RAIL_E_ASIC_P0V75_VDDPHY_HBM1357, 1500, 1500, 200, 940, 940, 525, 940 },
	{ VR_RAIL_E_ASIC_P0V75_OWL_E_VDD, 1600, 1600, 200, 940, 940, 525, 940 },
	{ VR_RAIL_E_ASIC_P0V4_VDDQL_HBM1357, 530, 530, 200, 800, 520, 280, 520 },
	{ VR_RAIL_E_ASIC_P1V1_VDDQC_HBM1357, 3200, 3200, 200, 1320, 1320, 770, 1320 },
	{ VR_RAIL_E_ASIC_P1V8_VPP_HBM1357, 140, 140, 200, 1980, 1980, 1260, 1980 },
	...
	{ VR_RAIL_E_ASIC_P1V2_HAMSA_VDDHRXTX_PCIE, 100, 100, 200, 1320, 1320, 840, 1320 },
	{ VR_RAIL_E_ASIC_P0V85_HAMSA_VDD, 1000, 1000, 200, 1063, 1063, 595, 1063 },
	{ VR_RAIL_E_ASIC_P1V1_VDDQC_HBM0246, 3200, 3200, 200, 1320, 1320, 770, 1320 },
	{ VR_RAIL_E_ASIC_P1V8_VPP_HBM0246, 140, 140, 200, 1980, 1980, 1260, 1980 },
	{ VR_RAIL_E_ASIC_P0V4_VDDQL_HBM0246, 530, 530, 200, 800, 520, 280, 520 },
	{ VR_RAIL_E_ASIC_P0V75_VDDPHY_HBM0246, 1500, 1500, 200, 940, 940, 525, 940 },
	{ VR_RAIL_E_ASIC_P0V75_OWL_W_VDD, 1600, 1600, 200, 940, 940, 525, 940 },
	{ VR_RAIL_E_ASIC_P0V75_MAX_S_VDD, 660, 660, 200, 1130, 975, 525, 975 },
	{ VR_RAIL_E_ASIC_P0V9_OWL_W_TRVDD, 170, 170, 200, 1130, 1130, 630, 1130 },
	{ VR_RAIL_E_ASIC_P0V75_OWL_W_TRVDD, 110, 110, 200, 1130, 975, 525, 975 },
};

const vr_test_mode_setting_t vr_test_mode_table_default[] = {
	// vr_rail, fast ocp: x/10(A), slow ocp: x/10(A), uvp: 1(mV), ovp: 1(mV), v max: 1(mV), lcr(mV), ucr(mV)
	{ VR_RAIL_E_ASIC_P0V85_MEDHA0_VDD, 14500, 13600, 640, 940, 918, 782, 918 },
	{ VR_RAIL_E_ASIC_P0V85_MEDHA1_VDD, 14500, 13600, 640, 940, 918, 782, 918 },
	...
	{ VR_RAIL_E_ASIC_P0V4_VDDQL_HBM1357, 460, 360, 325, 800, 440, 368, 440 },
	{ VR_RAIL_E_ASIC_P1V1_VDDQC_HBM1357, 2800, 2700, 900, 1260, 1166, 1034, 1166 },
	{ VR_RAIL_E_ASIC_P1V8_VPP_HBM1357, 140, 140, 1400, 1960, 1908, 1692, 1908 },
	...
	{ VR_RAIL_E_ASIC_P1V8_VPP_HBM0246, 140, 140, 1400, 1960, 1908, 1692, 1908 },
	{ VR_RAIL_E_ASIC_P0V4_VDDQL_HBM0246, 460, 360, 325, 800, 440, 368, 440 },
...
static bool set_vr_test_mode_reg(bool is_default)
{
	...
	for (uint8_t i = 0; i < table_size; i++) {
		...
		// check mode is test mode
		if (is_default == false) {
			// set FIXED OV / UV 14-bit to 1
			//check fixed_ov_uv_conf
			uint8_t tbuf[2], rbuf[4];
			uint32_t fixed_ov_uv_conf = 0;
			tbuf[0] = 0xE0;
			tbuf[1] = 0xF0;

			if (raa_dma_rd(bus, addr, tbuf, rbuf) < 0) {
				LOG_ERR("Read fixed_ov_uv_conf failed from dev: 0x%x", addr);
				return false;
			}
			memcpy(&fixed_ov_uv_conf, rbuf, sizeof(uint32_t));
		...
		vout_range_user_settings.change_vout_max[i] = cfg->ucr - 10;
	}
	return ret;
}
