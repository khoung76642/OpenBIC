...
#define ASIC_COM_TEST_REG 0x7A
	if (offset == ASIC_COM_TEST_REG) {
		uint8_t reply_buf[82] = { 0x50, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E,
...
#define ASIC_COM_FW_VERSION_REG 104
	if (offset == ASIC_COM_FW_VERSION_REG) {
		uint8_t reply_buf[11] = { 0x0B, 0x00, 20, 26, 0x02, 0x04, 0x05, 0x03, 26, 20, 0x06 };
		data->target_rd_msg.msg_length = sizeof(reply_buf);
		memcpy(data->target_rd_msg.msg, reply_buf, sizeof(reply_buf));
		LOG_HEXDUMP_DBG(reply_buf, sizeof(reply_buf), "ASIC_COM_TEST_REG reply");
...
#define SVS_CORE_VOLTAGE_REG 0x9B
	...
	case MONITOR:
		...
		uint8_t reply_data1[RETURN_LEN] = {0, 22, 23, 24, 20, 28, 33, 0, 0, 0};
	...
	case STAUTUS:
		...
		//return 8 byte
		struct_size = 8;
		uint8_t reply_data2[8] = {0, 0x10, 0, 0, 0, 0, 0, 0};;
