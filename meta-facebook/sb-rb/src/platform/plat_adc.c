...
typedef struct {
	...
	uint16_t ucr; // pwr
	float vr_voltage;
	float pwr_buf[ADC_AVERGE_TIMES_MAX];
	float pwr_avg_val;
	float pwr_sum;
...
static void adc_poll_init()
{
	for (uint8_t i = ADC_IDX_MEDHA0_1; i < ADC_IDX_MAX; i++) {
		...
		adc_info[i].pwr_sum = 0;
		adc_info[i].pwr_avg_val = 0;
		adc_info[i].vr_voltage = 0;
		memset(adc_info[i].buf, 0, sizeof(uint16_t)*ADC_AVERGE_TIMES_MAX);
		memset(adc_info[i].pwr_buf, 0, sizeof(float)*ADC_AVERGE_TIMES_MAX);
	}
}
...
static void update_adc_info(uint16_t raw_data, uint8_t base_idx, float vref)
{
	...
	for (uint8_t i = base_idx; i < ADC_IDX_MAX; i+=2)
	{
		...
		if (base_idx == ADC_RB_IDX_MEDHA0)
			adc->vr_voltage = get_sensor_reading_cache_as_float(SENSOR_NUM_ASIC_P0V85_MEDHA0_VDD_VOLT_V) / 1000;
		else if (base_idx == ADC_RB_IDX_MEDHA1)
			adc->vr_voltage = get_sensor_reading_cache_as_float(SENSOR_NUM_ASIC_P0V85_MEDHA1_VDD_VOLT_V) / 1000;
		adc->pwr_sum -= adc->pwr_buf[adc->buf_idx];
		adc->pwr_buf[adc->buf_idx] = adc->vr_voltage * adc_raw_mv_to_apms(adc->buf[adc->buf_idx], vref);
		adc->pwr_sum += adc->pwr_buf[adc->buf_idx];
		adc->pwr_avg_val = adc->pwr_sum / adc->avg_times;
...
int ad4058_write_reg(uint8_t reg, uint8_t write_val, uint8_t idx)
{
	...
	LOG_INF("medha%d ad4058 write reg 0x%x", idx, reg); ;
	return 0;
}
...
void adc_rainbow_polling_handler(void *p1, void *p2, void *p3)
{
	...
	LOG_WRN("for write/read test, no init");
	while (1) {
		if (adc_poll_flag)
		{
			switch (adc_idx)
