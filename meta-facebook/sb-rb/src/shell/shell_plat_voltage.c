...
static int cmd_voltage_set(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (get_svs_flag() == 1) {
		if (rail == VR_RAIL_E_ASIC_P0V85_MEDHA0_VDD || rail == VR_RAIL_E_ASIC_P0V85_MEDHA1_VDD) {
			// if svs is enable, millivolt need to + vout ofset
			uint16_t offset = 0;
			voltage_offset_get(rail, &offset);
			millivolt += offset;
		}
	}
...
static void cmd_svs_flag_set(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (svs_flag > 1) {
		shell_error(shell, "Invalid voltage offset value: %d. Only 0 or 1 is allowed.", svs_flag);
		return;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_voltage_cmds,
			       ...
			       SHELL_CMD_ARG(set, &voltage_rname,
					     "set <voltage-rail> <new-voltage> [perm]",
					     cmd_voltage_set, 3, 1),
